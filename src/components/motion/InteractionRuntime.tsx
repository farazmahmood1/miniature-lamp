...
/**
 * Resolve a spec node id against the DOM.
 *
 * The routes were captured at different times and emitted two different anchor
 * attributes for the same purpose. Trying both lets one runtime serve every page
 * without rewriting the specs those pages already ship.
 */
const byId = (id: string): HTMLElement | null =>
  document.querySelector<HTMLElement>('[data-cid="' + id + '"]') ??
...
// Apply a panel's open-state descendant overrides (node id to style). Reveals content whose
// visibility is gated by a JS-toggled class the static markup does not carry (e.g. Elementor
// e-active). Applied only when the panel is shown; the panel's own hide masks it.
function applyDesc(d?: Record<string, CapStyle>) {
  if (!d) return;
  for (const id in d) applyStyle(byId(id), d[id]);
}

/** Reproduces a captured interactive pattern by toggling captured inline styles on
 *  the existing DOM nodes (found by their captured node id). Renders nothing, and applies NOTHING
...
export default function InteractionRuntime({ spec }: { spec: Spec }) {
  useEffect(() => {
    ...
    if (spec.kind === "tabs") {
      ...
      const render = () => spec.tabs.forEach((t, i) => {
        const on = i === active;
        const trig = byId(t.trigger), panel = byId(t.panel);
      ...
      spec.tabs.forEach((t, i) => {
        ...
        trig.addEventListener("keydown", (e) => {
          ...
          if (k === "ArrowRight" || k === "ArrowLeft") {
            ...
            render();
            byId(spec.tabs[active].trigger)?.focus();
          }
        }, { signal });
      });
      ...
      const renderItem = (i: number) => {
        ...
        const trig = byId(it.trigger), region = byId(it.region);
      ...
      let index = spec.base;
      const track = byId(spec.track);
      const go = (k: number) => {
        index = Math.max(0, Math.min(n - 1, k));
        if (track) track.style.transform = spec.transforms[index];
        spec.bullets.forEach((b, bi) => applyStyle(byId(b), bi === index ? spec.bulletOn : spec.bulletOff));
      };
      const nextEl = spec.next ? byId(spec.next) : null;
      const prevEl = spec.prev ? byId(spec.prev) : null;
      ...
      spec.bullets.forEach((b, bi) => byId(b)?.addEventListener("click", (e) => { e.preventDefault(); go(bi); }, { signal }));
      ...
      spec.items.forEach((it) => {
        const trig = byId(it.trigger), panel = byId(it.panel);
        ...
        it.closes.forEach((c) => byId(c)?.addEventListener("click", (e) => { e.preventDefault(); set(false); }, { signal }));
